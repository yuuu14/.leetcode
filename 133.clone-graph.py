...
class Solution(object):
    def cloneGraph1(self, node): # DFS iteratively
        if not node:
            return node
        # hashtable from original to clone
        m = {node: Node(node.val)}
        stack = [node]
        while stack:
            # pop out most recent visited node
            # LIFO
            n = stack.pop()
            for neigh in n.neighbors:
                # check neighbor node visited or not
                if neigh not in m:
                    stack.append(neigh)
                    # add in hashtable, marked as well
                    m[neigh] = Node(neigh.val)
                # update neighbor for clone graph
